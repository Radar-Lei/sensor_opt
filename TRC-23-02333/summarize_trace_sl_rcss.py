...
def main():
    ...
    if not metrics:
        raise SystemExit(f"No seed metrics found under {input_root}")
    ...
    for budget, sub in pivot.groupby(level="budget"):
        ...
        row = {"budget": budget}
        for layout in ["rcss_selected", "robust_coverage_cvar"]:
            if layout in sub.columns:
                for baseline in ["random", "best_random_by_validation", "greedy_a_trace", "swap_from_scenario_cvar", "multistart_swap_by_validation", "top_variance"]:

...
OUTPUT_ROOT="${OUTPUT_ROOT:-TRC-23-02333/trace_sl_results/stage16_calibrated_trace_sweep/seattle_10_20_30_trainval}"
...
for seed in ${SEEDS}; do
  ...
  if [ -f "${metrics_path}" ]; then
    printf '[%s] skip completed Seattle Stage16 seed %s (%s)\n' "$(date -Is)" "${seed}" "${metrics_path}"
    continue
  fi

...
Table~\ref{tab:trace-biopt-problem-contract} turns that definition into a
reviewer-facing problem contract before the method details begin. The point is
to make explicit that Section~3 is about one deployment-time network-design
decision, one hidden-state recoverability target, one bilevel stochastic
structure, one split discipline, and one external audited comparison class,
not a generic sparse-sensor benchmark setup.

\input{tables/table_trace_biopt_problem_contract}
...
\caption{TRACE-BiOpt workflow. The design protocol separates training-derived reconstruction ingredients, bilevel layout optimization on validation data, and held-out deployment evaluation.}
\label{fig:workflow}
\end{figure}

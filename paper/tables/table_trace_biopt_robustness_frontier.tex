...
\bottomrule
\end{tabular}
\par\vspace{4pt}{\raggedright\footnotesize\textit{Note:} These rows remain bounded Stage\,14 stress-test evidence only. They do not assert TRACE-BiOpt robustness under the same perturbations because the Stage\,14 artifact evaluates pre-TRACE-BiOpt reviewer-facing baselines on two split seeds; the table is used to summarize how the stress frontier moves across perturbation types.\par}
\end{table*}

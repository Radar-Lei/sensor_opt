# Requirements Archive: v1.0 TRACE-SL Readiness
...
- [x] **CLAIM-01**: The paper states TRACE-SL as a transparent reconstruction-aware sensor placement problem, not as an ad hoc candidate-pool heuristic.
- [x] **CLAIM-02**: Every primary contribution claim maps to explicit evidence: held-out experiment, robustness result, statistical comparison, theorem/derivation, or limitation note.
- [x] **CLAIM-03**: The main performance claim avoids “best at all budgets” wording and correctly handles the 10% PeMS7_228 multistart-vs-RCSS caveat.
- [x] **CLAIM-04**: The paper uses “certificate-guided” or equivalent wording unless a formal certificate theorem is added.
- [x] **CLAIM-05**: The positioning distinguishes TRACE-SL from deterministic full-observability TSLP and from black-box traffic imputation/forecasting.
...
- [x] **BASE-01**: The experiment suite includes observability-inspired or TSLP-style baselines, with clear explanation that their objective differs from reconstruction-error minimization.
- [x] **BASE-02**: A-optimal and D-optimal design baselines are reported as independent baselines, not only candidate sources inside RCSS.
- [x] **BASE-03**: Graph sampling or graph signal reconstruction baselines are included or explicitly justified as already represented by existing GSP-related comparisons.
- [x] **BASE-04**: QR/SVD/POD sparse sensor placement baseline is implemented and evaluated where compatible with the traffic matrix representation.
- [x] **BASE-05**: A simple learning-based reconstruction check is run or scoped with a documented reason if deferred.
- [x] **BASE-06**: Multistart validation refinement is treated as a strong comparator or predeclared portfolio member, not ignored after observing the 10% budget result.
